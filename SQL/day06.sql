...
select ulke,avg(yas) as ortalama_yas from insan group by ulke;--as koyarak stun ismini degistirdik
select ulke,round(avg(yas))as ortalama_yas from insan group by ulke;--tam sayi olsun diye roun dedik.

--almanya 2 tane oldugu halde bir tane getirdi.her ulkeden bir tane
...
select ulke,cinsiyet,maas, count (isim) as calisan_sayisi, round(avg(yas)) as yas_ortalamasi
from insan  where maas>30000 group by ulke,cinsiyet,maas ;
--round(avg(maas),2)==> maaslari virgulden sonra iki basamak gorunsun istiyorsak " ,2" yazariz.


/*===============================================================================
HAVING = sart belirtmek icin kullanilir.
group by komutundan sonra yer alir.
Where komutundan farki: Gruplama yaptiktan sonra cikan sonucta sart kosacaksak o zaman Having yapilir.
===================================================================================*/


--SORU08: ulke'lere göre yas ortalamasını bulup, yas ortalaması 33'den buyuk olanları listeleyiniz. 
select ulke, round(avg(yas)) as ortalama_yas from insan group by ulke having avg(yas)>33; 

--SORU9: ulke'lere göre ortalama maası bulup, ortalama maas 30000'den buyuk olanları listeleyiniz 
select ulke, round(avg(maas),2) as ortalama_maas from insan group by ulke having avg(maas)>30000;
...
/*========================UNION, UNION ALL============================================
    ...
    UNION: SADECE BENZERSİZ VERİLERİ ALIR -ayni olanlardan sadece bir tanesini alir.
...
union
select maas,sehir from personel where maas>5000;--tek liste olarak alt alta siraladi verdi.
