...
------------------------------------------------------------------------------*/ 
select maas,isim,sirket from personel where maas<5000
union
select maas,isim,sirket from personel where sirket!='Honda';--her ikisini de calistiriyor, cift olanlardan 1 tane alyor.

select maas,isim,sirket from personel where maas<5000
union all
select maas,isim,sirket from personel where sirket!='Honda';--her iksinin de sonucunu yazdirir.
...
ilike==> hem a hem b ile baslayanlar seklinde sorgu yapar.
Örnek:
-----
...
/* ==================================  REGEXP_LIKE  ==========================================
   ...
   REGEXP_LIKE: Gibi anlamına gelir. Daha karmaşık sorgulama işlemi için kullanılabilir.
	...
	SELECT * FROM tablo_adi WHERE sutun_adi ~ PATTERN
...
select * from exercises where kelime ~* 'h[ai]t';--  "~ * "==>demek buyuk kucuk gozetmeksizin
